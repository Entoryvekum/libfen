...
template <int maxLength, int numKeys> class LocalTabuOptimizer
{
public:
    ...
    LocalTabuOptimizer(mapping<maxLength, numKeys> &ed, bool print) : encoding(ed), gen(std::random_device{}()), print(print) {
        // tabuList[radicalIdx][keyIdx] 存储该移动解禁的迭代轮次
        tabuList.resize(encoding.numRadical + 1, std::vector<int>(numKeys + 1, 0));
    }

    void solve(TabuParameters param, const std::vector<int> &available) {
        ...
        for (int iter = 1; iter <= param.maxIterations; ++iter) {
            ...
            if (print && iter % 100 == 0) {
                ...
                std::cout << std::format("\r[Iter: {} | Best: {} | Current: {} | Time: {}s]", iter, bestEnergy, currentEnergy,
                                         elapsed)
                          << std::flush;
            }
        }
