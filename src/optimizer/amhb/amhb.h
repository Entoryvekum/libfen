...
public:
    ...
    public:
        ...
        int calTask(std::vector<std::unique_ptr<AMHBWorker>> &allWorkers) {
            // LogSumExp计算基础权重
    ...
    struct AMHBWorker
    {
        AMHBWorker(int stealThreshold, AMHBOptimizer &optimizer,std::atomic<bool> &allowRun, std::atomic<int> &usedWorker, std::atomic<int> &workerDone,
            ...
            : stealThreshold(stealThreshold), optimizer(optimizer), globalAllowRun(allowRun), globalUsedWorker(usedWorker),
        ...
        void run() {
            while (true) {
                ...
                if(isLastBatch) {
                    // 等待所有worker结束
                    while (globalUsedWorker.load(std::memory_order_acquire) != globalWorkerDone.load(std::memory_order_acquire))
                        cpu_pause();
                    // 汇总
                    optimizer.reduce();
                }
        ...
        // 运行数据
        int availableRst = 0;
        alignas(128) std::atomic<int> left{0};
        alignas(128) std::atomic<int> right{0};
        alignas(128) std::atomic<bool> allowSteal{false};
        alignas(128) std::atomic_flag stealLock = ATOMIC_FLAG_INIT;
        int localLeft;
        int pos;
        ...
        // 优化器
        AMHBOptimizer &optimizer;
    private:
        int getStepSize(int l, int r) {
            const int &n = r - l;
            return std::clamp(static_cast<int>(std::floor(std::sqrt(n))), 1, 32);
        }
        bool runLocalTasks() {
            ...
            while (true) {
                ...
                if (pos >= localLeft) {
                    ...
                    if (localRight - localLeft >= stepSize + stealThreshold * 2) {
                        localLeft += stepSize;
                        left.store(localLeft, std::memory_order_seq_cst);
                        // 重新判断right-left是否达到锁定阀值
                        if (right.load(std::memory_order_seq_cst) - localLeft < stealThreshold * 2)
                            if (!lockFinalBatch())
                                break;
                    }
                    // 余量不够，锁定最后一批
        ...
        bool shrink() {
            if (localLeft > pos + 1) {
                localLeft = (pos + localLeft + 1) / 2;
                left.store(localLeft, std::memory_order_seq_cst);
                // 若回退后剩余任务个数重新回升到阀值以上，则需更改allowSteal
                if (right.load(std::memory_order_seq_cst) - localLeft >= stealThreshold * 2 && !localAllowSteal) {
                    // 加锁并等待加锁完成
                    while (stealLock.test_and_set(std::memory_order_acquire))
                        cpu_pause();
                    localAllowSteal = true;
    ...
    AMHBOptimizer(mapping<maxLength, numKeys> &ed, int numWorker, bool print, int totalNeighbors, int stealThreshold)
        : pool(numWorker),print(print), LiteOperatorPool(totalNeighbors), allowRun(false), allTaskComplete(false) {
        for (int i = 0; i < numWorker; ++i) {
            // 将所有需要的全局变量和引用传入 Worker
            workers.push_back(std::make_unique<AMHBWorker>(stealThreshold, *this,allowRun, usedWorker, workerDone, totalTask,
    ...
    void engage() {
        // 准备任务
        int totalGeneratedTasks = LiteOperatorPool.calTask(workers);
        OperatorResultType *bestCandidate = nullptr;
    ...
    void reduce() {
        ...
        if (print && iter % 10000 == 0) {
            ...
            std::cout << std::format("\n[Iter: {} | Temp: {} | Best: {} | Current: {} | Time: {:.2f}s ]", iter, T, bestEnergy,
                                     energyCur, elapsed)
                      << std::endl;
            for (int i = 0; i < LiteOperatorPool.operators.size(); i++)
                std::cout << std::format("[ {}: tempCoef: {:.4f} VaR: {:.4f} Prob: {:.4f} ] ", i, LiteOperatorPool.tempCoef[i],
                                         LiteOperatorPool.VaR[i], LiteOperatorPool.weight[i]);
            std::cout << std::endl;
        }
...
private:
    ...
    alignas(128) std::atomic<int> totalTask;
    alignas(128) std::atomic<int> usedWorker;
    alignas(128) std::atomic<int> workerDone;
    alignas(128) std::atomic<bool> allowRun;
    alignas(128) std::atomic<bool> allTaskComplete;
    AMHBParameters param;
    std::function<double(double, int, double)> nextTemp;
    double T;
    int iter;
    int bestEnergy;
    int energyCur;
